...
export default function DashboardHome() {
  ...
  const getRawDisbursementDate = (startDate, frequency, type) => {
    if (!startDate) return "";
    const d = new Date(startDate);
    if (type === "day") {
      d.setDate(d.getDate() - Number(frequency || 1));
    } else if (type === "month") {
      d.setMonth(d.getMonth() - Number(frequency || 1));
    }
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      loansSnap.forEach((d) => {
        ...
        const rawDisbDate = getRawDisbursementDate(
          data.startDate,
          data.frequency,
          data.frequencyType,
        );
        ...
        if (rawDisbDate >= firstDayOfMonth && rawDisbDate <= lastDayOfMonth) {
          ...
          const profit =
            data.totalProfit || Math.max(0, totalAmount - principal);
      ...
      let monthlyProfit = 0;
      const detailedMonthlyProfitsList = [];
      ...
      transMonthSnap.forEach((doc) => {
        ...
        const profitFromInst = data.profitShare || 0;
        const profitFromPenalty = data.penalty || 0;
        const totalProfitPerBill = profitFromInst + profitFromPenalty;

        if (totalProfitPerBill > 0) {
          monthlyProfit += totalProfitPerBill;

          detailedMonthlyProfitsList.push({
            id: doc.id,
            paymentDate: data.paymentDate,
            customerName: data.customerName,
            loanNumber: lNum,
            profitShare: profitFromInst,
            penalty: profitFromPenalty,
            totalProfit: totalProfitPerBill,
            customerId: data.customerId,
          });
        }
      });

      detailedMonthlyProfitsList.sort((a, b) => {
      ...
      setStats({
        ...
        totalActiveProfit: totalActiveProfit,
      ...
      setModalData({
        ...
        monthlyProfits: detailedMonthlyProfitsList,
      });
    } catch (error) {
    ...
    <div className="w-full pb-20 px-4 md:px-8 font-sans animate-in fade-in duration-500 relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 mb-8">
        ...
        >
          ...
          <p className="text-[12px] xl:text-[10px] font-black uppercase text-gray-400 tracking-widest mb-1 flex items-center gap-1">
            ยอดปล่อย (เดือนนี้) <FileText className="w-3 h-3 text-gray-300" />
          </p>
        ...
        >
          ...
          <p className="text-[12px] xl:text-[10px] font-black uppercase text-gray-400 tracking-widest mb-1 flex items-center gap-1">
            ยอดคงเหลือในตลาด <FileText className="w-3 h-3 text-gray-300" />
          </p>
          <p className="text-2xl font-black text-gray-800">
        ...
        >
          ...
          <p className="text-[12px] xl:text-[10px] font-black uppercase text-green-600 tracking-widest mb-1 flex items-center gap-1">
        ...
        >
          ...
          <p className="text-[12px] xl:text-[10px] font-black uppercase text-orange-400 tracking-widest mb-1 flex items-center gap-1">
          ...
          <div className="relative bg-white w-full max-w-6xl rounded-[2.5rem] shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200 overflow-hidden">
              ...
              <div>
                <h2 className="text-xl md:text-2xl font-black text-gray-800 flex items-center gap-3">
                  {activeModal === "disbursed" && (
                    <>
                      <Banknote className="w-6 h-6 text-purple-500" />{" "}
                      ยอดปล่อยรวมประจำเดือน
                    </>
                  )}
                  {activeModal === "principal" && (
                    <>
                      <Wallet className="w-6 h-6 text-blue-500" />{" "}
                      ยอดเงินคงเหลือในตลาด
                    </>
                  )}
                  {activeModal === "target" && (
                    <>
                ...
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">
                  {activeModal === "disbursed"
                    ? `แสดงยอดปล่อยกู้เฉพาะเดือน ${new Date(selectedDate).toLocaleDateString("th-TH", { month: "long", year: "numeric" })} (เรียงตามวันที่ปล่อย)`
                    : activeModal === "principal"
                      ? "แสดงวงกู้ที่กำลังดำเนินการ แยกตามเงินต้น ดอกเบี้ย และยอดรวม"
                      : activeModal === "profitMonth"
                        ? `กางบิลกำไรทุกใบ ประจำเดือน ${new Date(selectedDate).toLocaleDateString("th-TH", { month: "long", year: "numeric" })}`
              ...
              <table className="w-full text-left whitespace-nowrap">
                <thead className="sticky top-0 bg-white shadow-sm z-10">
                  <tr className="text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">
                    {/* 🌟 หัวตารางสำหรับ "กำไรรายเดือน" */}
                    {activeModal === "profitMonth" ? (
                      <>
                        <th className="px-6 py-4 text-center w-16">#</th>
                        <th className="px-6 py-4 text-center">วันที่รับเงิน</th>
                        <th className="px-6 py-4">ชื่อลูกค้า</th>
                        <th className="px-6 py-4 text-center">วงที่</th>
                        <th className="px-6 py-4 text-right">กำไรจากค่างวด</th>
                        <th className="px-6 py-4 text-right">กำไรจากค่าปรับ</th>
                        <th className="px-6 py-4 text-right text-orange-500">
                          รวมกำไรที่ได้
                        </th>
                      </>
                    ) : /* 🌟 หัวตารางสำหรับ "ยอดเงินคงเหลือในตลาด (Active Loans)" */
                    activeModal === "principal" ? (
                      <>
                    ...
                    ) : /* 🌟 หัวตารางสำหรับ "ยอดปล่อยกู้ (Disbursed)" */
                    activeModal === "disbursed" ? (
                      <>
                        <th className="px-6 py-4">#</th>
                        <th className="px-6 py-4">ชื่อลูกค้า</th>
                        ...
                        <th className="px-6 py-4 text-right">เงินต้น</th>
                        <th className="px-6 py-4 text-right text-gray-500">
                          รับจริง (รวมดอก)
                        </th>
                        <th className="px-6 py-4 text-right text-green-500">
                ...
                <tbody className="divide-y divide-gray-50">
                  {/* --- ตาราง: ยอดปล่อย --- */}
                  {activeModal === "disbursed" &&
                    modalData.disbursedLoans.map((item, idx) => (
                      <tr
                        key={item.id}
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-center">
                          <Link
                            ...
                            className="inline-flex items-center justify-center p-2 bg-purple-50 text-purple-600 rounded-lg hover:bg-purple-600 hover:text-white transition-colors"
                    ...
                    modalData.dailyTargets.map((item, idx) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-center">
                          <Link
                            href={`/customers/${item.customerId}`}
                            className="inline-flex items-center justify-center p-2 bg-orange-50 text-orange-500 rounded-lg hover:bg-orange-500 hover:text-white transition-colors"
                            title="ดูโปรไฟล์ลูกค้า"
                          >
                            <ChevronRight className="w-5 h-5" />
                          </Link>
                    ...
                    modalData.dailyTargets.map((item, idx) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-center">
                          <Link
                            href={`/customers/${item.customerId}`}
                            className="inline-flex items-center justify-center p-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-600 hover:text-white transition-colors"
                            title="ดูโปรไฟล์ลูกค้า"
                          >
                            <ChevronRight className="w-5 h-5" />
                          </Link>
                  ...
                  {/* --- ตาราง: ดีเทลไส้ในของกำไรรายเดือน --- */}
                  {activeModal === "profitMonth" &&
                    modalData.monthlyProfits.map((item, idx) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-xs font-bold text-gray-500 text-center whitespace-nowrap">
                          {formatDateThai(item.paymentDate)}
                        </td>
                        <td className="px-6 py-4 font-black text-gray-700">
                          <Link
                            href={`/customers/${item.customerId}`}
                        ...
                        <td className="px-6 py-4 text-sm font-black text-gray-500 text-center">
                          {item.loanNumber || "-"}
                        </td>
                        <td className="px-6 py-4 font-bold text-green-500 text-right">
                          {item.profitShare > 0
                            ? `+฿${item.profitShare.toLocaleString()}`
                            : "-"}
                        </td>
                        <td className="px-6 py-4 font-bold text-rose-500 text-right">
                          {item.penalty > 0
                            ? `+฿${item.penalty.toLocaleString()}`
                            : "-"}
                        </td>
                        <td className="px-6 py-4 font-black text-orange-600 text-right bg-orange-50/30">
                  ...
                  {((activeModal === "principal" &&
                    ...
                    (activeModal === "disbursed" &&
                      modalData.disbursedLoans.length === 0) ||
                    (activeModal === "target" &&
            ...
            <div className="p-6 md:p-8 bg-[#1F2335] text-white flex flex-col md:flex-row md:justify-between items-start md:items-center rounded-b-[2.5rem] gap-4">
              <div>
                ...
                {/* 🌟 ข้อมูลสรุปใหม่สำหรับ ยอดคงเหลือในตลาด (ฝั่งซ้าย: ต้นทุนรวม) */}
                {activeModal === "principal" && (
                  <div className="mt-3">
                    <p className="text-[10px] font-bold text-gray-400 mb-1 uppercase tracking-widest">
                      รวมต้นทุนที่ปล่อยไป (เงินต้น)
                    </p>
                    <p className="text-3xl font-black text-white">
                      ฿{stats.totalActivePrincipal.toLocaleString()}
                    </p>
                    <p className="text-[10px] font-bold text-gray-400 mt-1 flex items-center gap-2">
                      จากทั้งหมด{" "}
                      <span className="text-blue-400 font-black">
                        {modalData.activeLoans.length} วง
                      </span>{" "}
                      • ดอกเบี้ย{" "}
                      <span className="text-green-400 font-black">
                        +฿{stats.totalActiveProfit.toLocaleString()}
                      </span>
                    </p>
                  </div>
                )}
              ...
              <div className="text-left md:text-right w-full md:w-auto">
                {activeModal === "disbursed" ? (
                  ...
                  // 🌟 ข้อมูลสรุปใหม่สำหรับ ยอดคงเหลือในตลาด (ฝั่งขวา: ยอดคงเหลือ)
